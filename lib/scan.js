...
/** Docker socket 的常见位置。 */
const DOCKER_SOCKETS = ["/var/run/docker.sock", `${process.env.HOME ?? ""}/.docker/run/docker.sock`];

/**
 * Docker 是否很可能在跑：先看 socket，避免为一个没装的 Docker 白等 2.5s。
 *
 * 刻意不做 try/catch：这里只会调用 `existsSync`（对任何路径都不抛），把异常吞掉
 * 曾经让「忘了 import」这类错误变成静默失效（Docker 支持直接变死代码）。
 * @param {string[]} [sockets] - 候选 socket 路径（测试可注入）。
 * @returns {boolean} 是否值得调用 docker。
 */
export function dockerLikelyRunning(sockets = DOCKER_SOCKETS) {
  return sockets.some((socket) => socket !== "" && existsSync(socket));
}
...
/**
 * 从可执行路径里取 `.app` 包名（任意层级，不只 /Applications）。
 * @param {string} exe - 可执行文件路径。
 * @returns {string|null} 包名。
 */
export function bundleNameOf(exe) {
 ...
 * 从命令行推断友好名字、进程类别与徽标。
 *
 * 判定顺序是有讲究的：先看容器，再看 `/Applications` 里的 GUI 应用，然后才是解释器
 * （`python -m uvicorn` 这种要显示成“python · uvicorn”，而不是把它当成 `Python.app`），
 * 其它位置的 `.app` 放在解释器之后兜底。
...
export function describeProcess({ name, command, args, container }) {
  ...
  // `.app` 里的可执行文件路径要从**原始命令行**里抠：macOS 的应用路径常带空格
  // （`/Applications/Visual Studio Code.app/...`），按空白切分出来的 args[0] 会断在空格上。
  const appExe = /^\s*(.+?\.app\/Contents\/MacOS\/\S+)/.exec(command)?.[1] ?? exe;
  // GUI 应用包优先（/Applications 或路径开头的 .app）。
  const topBundle = /\/Applications\/([^/]+)\.app\/|^([^/]+)\.app\//.exec(appExe);
  if (topBundle !== null) {
    return { title: (topBundle[1] ?? topBundle[2] ?? "").replace(/\.app$/, ""), kind: "app", badge: "App", detail: appExe };
  }
  if (["node", "deno", "bun", "tsx", "ts-node"].includes(lower)) {
  ...
  if (/^python[\d.]*$/.test(lower)) {
    const script = firstScriptArg(args.slice(1));
    // `-m uvicorn` / `manage.py` 这类：把真正跑的东西带上，比光一个 python 有用得多。
    if (script !== null) return { title: `${lower} · ${script.split("/").pop()}`, kind: "python", badge: "Python", detail: script };
    return { title: lower, kind: "python", badge: "Python", detail: command };
  }
  if (/^ruby[\d.]*$/.test(lower)) return { title: lower, kind: "other", badge: "Ruby", detail: command };
  if (/^(java|go|php|dotnet|cargo|rustc)$/.test(lower)) return { title: lower, kind: "other", badge: lower, detail: command };
  if (/^(docker-proxy|com\.docker|containerd|dockerd)/.test(lower)) {
    return { title: base, kind: "docker", badge: "Docker", detail: command };
  }
  if (/^(mysqld|postgres|redis-server|mongod|nginx|httpd|apache2|mariadbd?)$/.test(lower)) {
    return { title: base, kind: "service", badge: "Service", detail: command };
  }
  // 非 /Applications 位置的 .app（例如 /Library/Frameworks/.../Python.app 之外的 GUI 包）。
  const lateBundle = bundleNameOf(appExe);
  if (lateBundle !== null) return { title: lateBundle, kind: "app", badge: "App", detail: appExe };
...
/**
 * 把一条命令行缩成短名（取第一个 token 的 basename）。
 *
 * 用来展示父进程链之类的场景：`ps -o comm=` 在 macOS 上会被截断到 16 字符
 * （`/Users/youngi/.h`），所以宁可拿完整 `command=` 再自己缩。
 * @param {string} command - 命令行。
 * @returns {string} 短名。
 */
export function displayNameOf(command) {
