...
import { displayNameOf, isSystemBinary, protectionOf, runCommand, scanPorts, splitCommand } from "./scan.js";
...
/**
 * 取必填整数（只接受整数本身或纯数字字符串，`"646abc"` 不算）。
 ...
 */
function requireInt(payload, key, min, max) {
  ...
  const value = typeof raw === "number"
    ...
    : typeof raw === "string" && /^-?\d+$/.test(raw.trim())
      ...
      : Number.NaN;
  if (!Number.isSafeInteger(value)) throw new PortManagerApiError("bad-request", `${key} 必须是整数`);
  if (min !== undefined && value < min) throw new PortManagerApiError("bad-request", `${key} 不能小于 ${min}`);
  if (max !== undefined && value > max) throw new PortManagerApiError("bad-request", `${key} 不能大于 ${max}`);
  return value;
}

/** 端口号校验：1..65535。 */
function requirePort(payload, key) {
  return requireInt(payload, key, 1, 65_535);
}
...
/**
 * 结束前的策略校验：把「UI 上标成受保护」这件事在服务端也真正拦下来。
 *
 * 列表里的 `protected` 只是展示标记，不能当作安全边界：任何能发请求的人都可以直接
 * 调 `kill`。所以这里复用与扫描同一套判定（系统账号、系统目录程序、ControlCenter），
 * 并且**不信任请求里的 Host**——真正决定「这是不是我自己的进程」的是进程链：
 * 插件跑在 DSH 宿主进程里，所以 `process.pid` 及其祖先链一律拒绝结束。
 * @param {{pid:number, port:number, name:string, command:string, user:string|null, selfPort:number|null, selfChain:Set<number>}} input - 目标信息。
 * @returns {string|null} 拒绝原因；可结束时为 null。
 */
export function killRefusalReason({ pid, port, name, command, user, selfPort, selfChain }) {
  const chain = selfChain ?? new Set([process.pid, process.ppid]);
  if (chain.has(pid)) {
    return pid === process.pid ? "这是 DSH 自己的进程" : "这是 DSH 宿主所在进程链上的一员（结束了当前界面也会一起没）";
  }
  if (pid <= 1) return "系统启动进程";
  const selfUid = typeof process.getuid === "function" ? process.getuid() : null;
  const policy = protectionOf({ port, pid, name, user, selfPort, selfUid });
  if (policy !== null) return policy;
  if (isSystemBinary(command)) return "系统自带程序（位于 /System、/usr/libexec 等目录）";
  return null;
}

/** DSH 宿主进程链缓存：进程的祖先在生命周期内不变，算一次就够。 */
let selfChainCache = null;

/**
 * 取 DSH 宿主进程 + 其祖先 PID 集合（最多上溯 8 层）。
 * @returns {Promise<Set<number>>} PID 集合。
 */
export async function selfProcessChain() {
  if (selfChainCache !== null) return selfChainCache;
  const chain = new Set([process.pid]);
  let cursor = process.pid;
  for (let depth = 0; depth < 8; depth += 1) {
    const result = await runCommand("ps", ["-o", "ppid=", "-p", String(cursor)]);
    const ppid = Number.parseInt(result.stdout.trim(), 10);
    if (!Number.isInteger(ppid) || ppid <= 1) break;
    chain.add(ppid);
    cursor = ppid;
  }
  selfChainCache = chain;
  return chain;
}

/**
 * 校验某个 PID 是否仍然监听指定端口、属于当前用户，且不在保护名单里。
 *
 * kill 之前必须过这一关：列表可能是几秒前扫出来的，PID 可能已被系统复用。
 * @param {number} pid - 目标进程。
 * @param {number} port - 端口。
 * @param {number|null} [selfPort] - DSH 自己监听的端口（来自请求 Host，仅作补充判定）。
 * @returns {Promise<{pid:number, port:number, name:string, user:string|null, command:string}>} 校验结果。
 */
export async function verifyListener(pid, port, selfPort = null) {
  ...
  // lsof 的字段流里 `c` 是进程名、`p` 是 PID；只有确实在监听这个端口的 PID 才算数。
  ...
  const refusal = killRefusalReason({
    pid,
    port,
    name,
    command,
    user,
    selfPort,
    selfChain: await selfProcessChain(),
  });
  if (refusal !== null) {
    throw new PortManagerApiError("refused", `不能结束这个监听：${refusal}`, 403);
  }
  return { pid, port, name, user, command };
}
...
export function apply(ctx) {
   ...
   */
  async function dispatch(method, rawPayload, req) {
    // 请求体可以是 `{}`、也可以是 JSON `null`（readJsonBody 原样返回），统一兜底。
    ...
    switch (method) {
      ...
      case "detail": {
        const pid = requireInt(payload, "pid", 2);
        ...
        for (let depth = 0; depth < 4 && cursor > 1; depth += 1) {
          // 用 command= 而不是 comm=：macOS 的 comm 会被截断到 16 字符。
          const parent = await runCommand("ps", ["-o", "pid=,ppid=,command=", "-p", String(cursor)]);
          const parentMatch = /^\s*(\d+)\s+(\d+)\s+(.*)$/.exec(parent.stdout.trim());
          if (parentMatch === null) break;
          parents.push({ pid: Number.parseInt(parentMatch[1], 10), name: displayNameOf(parentMatch[3]) });
          cursor = Number.parseInt(parentMatch[2], 10);
      ...
      case "kill": {
        ...
        const verified = await verifyListener(pid, port, selfPort);
      ...
      case "open": {
        ...
        // Windows 走 `cmd /c start`，URL 里的 `&`/`|`/`^` 会被 cmd 当控制符解释。
        if (process.platform === "win32" && !/^[A-Za-z0-9\-._~/?#\[\]@!$'()*+,;=%]*$/.test(path)) {
          throw new PortManagerApiError("bad-request", "路径里含有 Windows 命令行不接受的字符");
        }
